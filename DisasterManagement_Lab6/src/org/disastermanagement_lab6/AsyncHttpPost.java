	...
	private void onQueryReportExecute(JSONArray Result) {
		...
		for (int i = 0; i < Result.length(); i++) {
			try {
				...
				mMap.addMarker(new MarkerOptions().position(latlng).title(report.getString("disaster").toUpperCase())
						.icon(setMapMarker(report_string)).snippet(setSnippet(report_string, report)));
